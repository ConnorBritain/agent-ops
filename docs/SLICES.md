...
> **What this is:** the menu, grouped by PI (Program Increment) with sprints nested under each. Each sprint is a named **slice** you can pick up cold: spin up a session, name the slice, and it reads that slice's prescribed read-order and self-orients. Slice names are the `/slice` keys, mirrored in [`docs/specs/SPEC-000-program-charter.md`](docs/specs/SPEC-000-program-charter.md), tracked in [`docs/status/current.md`](docs/status/current.md) + [`docs/traceability/phase-3-worker-runtime-core.yaml`](docs/traceability/phase-3-worker-runtime-core.yaml).
...
### PHASE-3-WORKER-SAFETY — Outbound worker runtime and host safeguards · 🟢 Active · ~4 sessions remaining
> Sprints: WORKER-RUNTIME-CORE ✅ · WORKER-SAFETY-HOOKS 🟡 · WORKER-SERVICE-PACKAGING ⚪ · PRIVATE-WORKER-CANARY 🔒
> Exec plan: WORKER-SAFETY-HOOKS→WORKER-SERVICE-PACKAGING→PRIVATE-WORKER-CANARY
...
| WORKER-SAFETY-HOOKS | `/slice worker-safety-hooks` | 🟡 Next · **P0** | ~2 | WORKER-RUNTIME-CORE | Implement resource, worktree, process, stale-session, and unsafe-delete policy hooks with dry-run remediation and independent monitor scheduling. |
...
- **Status:** 🟡 Next (PHASE-3-WORKER-SAFETY · WORKER-SAFETY-HOOKS)
...
- **Live status** → [`docs/status/current.md`](docs/status/current.md) + [`docs/traceability/phase-3-worker-runtime-core.yaml`](docs/traceability/phase-3-worker-runtime-core.yaml)
