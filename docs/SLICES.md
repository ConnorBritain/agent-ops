...
> **What this is:** the menu, grouped by PI (Program Increment) with sprints nested under each. Each sprint is a named **slice** you can pick up cold: spin up a session, name the slice, and it reads that slice's prescribed read-order and self-orients. Slice names are the `/slice` keys, mirrored in [`docs/specs/SPEC-000-program-charter.md`](docs/specs/SPEC-000-program-charter.md), tracked in [`docs/status/current.md`](docs/status/current.md) + [`docs/traceability/phase-4-roadmap-adapter.yaml`](docs/traceability/phase-4-roadmap-adapter.yaml).
...
### PHASE-4-ROADMAP-PRINT-PROVIDER — Roadmap composition and PrintProvider · 🟢 Active · ~2 sessions remaining
> Sprints: ROADMAP-ADAPTER ✅ · PRINT-PROVIDER ⚪
> Exec plan: PRINT-PROVIDER
...
- **Live status** → [`docs/status/current.md`](docs/status/current.md) + [`docs/traceability/phase-4-roadmap-adapter.yaml`](docs/traceability/phase-4-roadmap-adapter.yaml)
