...
injected transports and ledger ports. Phase 9 adds a human-observed browser
evidence boundary; it consumes only redacted human evidence and explicitly has
no browser, desktop, or scheduler-control path. All remain fixture boundaries.
...
| `packages/contracts` | Versioned command, job, event, provider, verification, external-projection, portable-primitive, estimation, allocation, release-recovery, human-observed-browser, and remote-access contracts. | I/O, credentials, raw browser/session facts, host/session facts, or current operational state. |
...
| `packages/policy` | Deterministic authorization, resource, destructive-delete, dry-run cleanup, safety, and exact-domain/browser-write-intent decisions. | Natural-language authority, secret storage, browser/desktop control, command execution, process control, or filesystem deletion. |
...
| `packages/providers/observed-browser` | No-execution handoff that records correlated, redacted evidence supplied by a human observer and a separate human confirmation record. | Browser launch/control/input, desktop or remote-access control, submission, provider/scheduler authority, raw content/session capture, or automatic restart. |
...
| `packages/test-kit` | Deterministic clocks, resources, control-plane recordings, release-recovery ledger and human-browser-evidence fakes, fixtures, contract suites, and acceptance harnesses. | Production credentials, host inventory, browser access, backup access, or service control. |
...
| `config/observed-browser.manifest.yaml` | Public human-observed browser maturity, domain, confirmation, redaction, and authority invariants. | Browser endpoints, accounts, session/device identifiers, remote-access topology, secret values, or execution. |
